...
function MultiRootDictCascader(props) {
  ...
  useEffect(() => {
    // 数据初始化
    const forMatValue = value.map(({ id, values = [] }) => ({
      rootId: id,
      childIds: values.map(({ id }) => id),
    }));

    if (forMatValue.childIds.length > 0) {
      // 数据做回显
    ...
    DictAssistant.fetchDictsWithMemo(dict).then((rows = []) => {
      const initOptions = rows.map(({ _id, name, code }) => ({
        value: _id,
        label: name,
        key: _id,
        code,
...
export default MultiRootDictCascader;
