...
const createClass = _createClass;
    ...
    createClass(Onion, [
      ...
      },
      {
        ...
        value: function execute() {
          const params = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;
          const fn = compose(
            [].concat(
...
const assertThisInitialized = _assertThisInitialized;
...
const possibleConstructorReturn = _possibleConstructorReturn;

const getPrototypeOf = createCommonjsModule(function(module) {
...
const setPrototypeOf = createCommonjsModule(function(module) {
...
const isNativeFunction = _isNativeFunction;

const construct = createCommonjsModule(function(module) {
...
const wrapNativeSuper = createCommonjsModule(function(module) {
    ...
    createClass(MapCache, [
      ...
      },
      {
        ...
        value: function set(key, value) {
          ...
          const ttl = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 60000;

          // 如果超过最大缓存数, 删除头部的第一个缓存.
          if (this.maxCache > 0 && this.cache.size >= this.maxCache) {
            const deleteKey = toConsumableArray(this.cache.keys())[0];
...
const RequestError =
  ...
  (function(_Error) {
    ...
    function RequestError(text, request) {
      ...
      const type =
        arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'RequestError';
...
function safeJsonParse(data) {
  const throwErrIfParseFail =
    arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
  const response = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : null;
  const request = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : null;
...
const { toString } = Object.prototype; // Check env is browser or node

function getEnv() {
  let env; // Only Node.JS has a process variable that is of [[Class]] process
...
// 前后缀拦截
const addfix = function addfix(url) {
...
let warnedCoreType = false; // 默认缓存判断，开放缓存判断给非 get 请求使用

function __defaultValidateCache(url, options) {
  const _options$method = options.method;
  const method = _options$method === void 0 ? 'get' : _options$method;
  return method.toLowerCase() === 'get';
}

function fetchMiddleware(ctx, next) {
  if (!ctx) return next();
  let _ctx$req = ctx.req;
  _ctx$req = _ctx$req === void 0 ? {} : _ctx$req;
  const _ctx$req$options = _ctx$req.options;
  const options = _ctx$req$options === void 0 ? {} : _ctx$req$options;
  const _ctx$req$url = _ctx$req.url;
  const url = _ctx$req$url === void 0 ? '' : _ctx$req$url;
  const { cache } = ctx;
  const { responseInterceptors } = ctx;

  const _options$timeout = options.timeout;
  const timeout = _options$timeout === void 0 ? 0 : _options$timeout;
  const _options$__umiRequest = options.__umiRequestCoreType__;
  const __umiRequestCoreType__ =
    _options$__umiRequest === void 0 ? 'normal' : _options$__umiRequest;
  const _options$useCache = options.useCache;
  const useCache = _options$useCache === void 0 ? false : _options$useCache;
  const _options$method2 = options.method;
  const method = _options$method2 === void 0 ? 'get' : _options$method2;
  const { params } = options;
  const { ttl } = options;
  const _options$validateCach = options.validateCache;
  const validateCache =
...
function parseResponseMiddleware(ctx, next) {
  ...
  return next()
    .then(function() {
      if (!ctx) return;
      const _ctx$res = ctx.res;
      const res = _ctx$res === void 0 ? {} : _ctx$res;
      const _ctx$req = ctx.req;
      const req = _ctx$req === void 0 ? {} : _ctx$req;

      const _ref = req || {};
      let _ref$options = _ref.options;

      _ref$options = _ref$options === void 0 ? {} : _ref$options;
      const _ref$options$response = _ref$options.responseType;
      const responseType = _ref$options$response === void 0 ? 'json' : _ref$options$response;
      const _ref$options$charset = _ref$options.charset;
      const charset = _ref$options$charset === void 0 ? 'utf8' : _ref$options$charset;
      const _ref$options$getRespo = _ref$options.getResponse;
      const _ref$options$throwErr = _ref$options.throwErrIfParseFail;
      const throwErrIfParseFail = _ref$options$throwErr === void 0 ? false : _ref$options$throwErr;
      const _ref$options$parseRes = _ref$options.parseResponse;
      const parseResponse = _ref$options$parseRes === void 0 ? true : _ref$options$parseRes;
    ...
    .then(function(body) {
      if (!ctx) return;
      const _ctx$res2 = ctx.res;
      const _ctx$req2 = ctx.req;
      const req = _ctx$req2 === void 0 ? {} : _ctx$req2;

      const _ref2 = req || {};
      let _ref2$options = _ref2.options;

      _ref2$options = _ref2$options === void 0 ? {} : _ref2$options;
      const _ref2$options$getResp = _ref2$options.getResponse;
      const getResponse = _ref2$options$getResp === void 0 ? false : _ref2$options$getResp;
...
function simplePostMiddleware(ctx, next) {
  ...
  const _options$requestType = options.requestType;
  const requestType = _options$requestType === void 0 ? 'json' : _options$requestType;
  const { data } = options; // 数据使用类axios的新字段data, 避免引用后影响旧代码, 如将body stringify多次
...
function paramsSerialize(params, paramsSerializer) {
  ...
  let jsonStringifiedParams; // 支持参数自动拼装，其他 method 也可用，不冲突
...
function simpleGetMiddleware(ctx, next) {
  ...
  const _ctx$req$options = _ctx$req.options;
  const options = _ctx$req$options === void 0 ? {} : _ctx$req$options;
  const { paramsSerializer } = options;
  const { params } = options;
  let _ctx$req2 = ctx.req;
  _ctx$req2 = _ctx$req2 === void 0 ? {} : _ctx$req2;
  const _ctx$req2$url = _ctx$req2.url;
  const url = _ctx$req2$url === void 0 ? '' : _ctx$req2$url; // 将 method 改为大写
      ...
      [
        ...
        },
        {
          ...
          value: function dealRequestInterceptors(ctx) {
            const reducer = function reducer(p1, p2) {
              return p1.then(function() {
                const ret = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
...
const request = function request() {
  ...
  const umiInstance = function umiInstance(url) {
    const options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
...
const extend = function extend(initOptions) {
