...
function projectCode({form,disabled}) {
  ...
  function merge() {
    ...
    form.setFieldsValue({code:isEnough(fv.mainCode,4) + isEnough(fv.sonCode,3) + isEnough(fv.grandsonCode,3) +
      isEnough(regionCode,6) + isEnough(fv.otherCode,8)})
  }
  ...
  function onChange(value) {
    ...
    if(data){
      ...
      form.setFieldsValue({name, mainCode:code && code.substring(0,4),
      ...
      form.setFieldsValue({mainCode:value && value.substring(0,4),
  ...
  function onSearch(value) {
    PROJECTOVERVIEWS.searchProjectOverviewUsingGET({params:{code:value}}).then((resp=[])=>{
      setOptions(resp)
    })
  }
